...
CUSTOM_API_CONFIG = {
    ...
    "model_name": "moonshotai/kimi-k2-0905",  # 🧪 测试 Kimi K2-0905 模型
    "max_tokens": 8192,
    "temperature": 0.7,
    "top_p": 0.8,
    ...
    def generate_reply(self, history: List[Dict]) -> str:
        ...
        for attempt in range(max_retries):
            try:
                ...
                # 🔧 启用 Gemini 2.5 Pro 的 thinking mode（深度思考模式）
                # OpenRouter 支持通过 extra_body 传递模型特定参数
                if "gemini" in self.config["model_name"].lower():
                    api_params["extra_body"] = {
                        "thinkingBudget": 32768  # 设置较高的思考预算以启用深度思考（范围：128-32768）
                    }
                    print(f"🔧 [TestModel] 为 Gemini 2.5 Pro 启用 Thinking Mode")
                    print(f"   API 参数: thinkingBudget = 32768 (深度思考)")
                
                # 调用API
                print(f"🤖 [TestModel] 正在调用 API: {self.config['model_name']}")
                response = self.client.chat.completions.create(**api_params)
                print(f"✅ [TestModel] API 调用成功，响应长度: {len(response.choices[0].message.content)} 字符")
