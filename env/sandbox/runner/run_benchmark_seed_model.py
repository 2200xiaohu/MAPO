...
# 火山方舟 Ark SDK（需要提前安装：pip install --upgrade "volcengine-python-sdk[ark]"）
...
# 🔧 在这里配置你的自定义模型API（火山方舟 Doubao 接入点）
CUSTOM_API_CONFIG = {
    "api_key": None,  # 优先从环境变量 ARK_API_KEY 读取
    "base_url": "https://ark.cn-beijing.volces.com/api/v3",
    # 这里使用的是方舟推理接入点 ID，对应的底层模型是 doubao-1.5-character-0228
    "model_name": "ep-20250307184613-44sf4",
    "max_tokens": 8192,
    "temperature": 0.7,
    "top_p": 0.8,
...
# 评测配置
# 使用 runner/benchmark_cases/ 下的补跑案例（10个）
BENCHMARK_CASES_FILE = PROJECT_ROOT / "runner/benchmark_cases/sampled_benchmark_30.txt"
OUTPUT_DIR = PROJECT_ROOT / "results/benchmark_runs"
MAX_TURNS = 45  # 每个对话最多轮次
K = 1  # 每K轮评估一次EPJ

# 并发配置
MAX_WORKERS = 1  # 并发线程数（建议2-5，太高可能触发API限流）

# 测试模式配置
TEST_MODE = False  # 🧪 测试模式：True = 只运行前N个案例，False = 运行全部30个
...
class CustomTestModel:
    ...
    def __init__(self, config: Dict):
        ...
        # 处理 API key（从环境变量 ARK_API_KEY 获取）
        ...
        if api_key is None:
            ...
            if not api_key:
                raise RuntimeError(
                    ...
                    "请先在终端中执行：export ARK_API_KEY=你的方舟API_KEY"
                )
        ...
        print(f"   模型(推理接入点ID): {config['model_name']}")
    ...
    def generate_reply(self, history: List[Dict]) -> str:
        ...
        for attempt in range(max_retries):
            try:
                ...
                # 构建 OpenAI/Ark 兼容格式的消息
                messages = [
                    {"role": "system", "content": system_prompt},
                    {"role": "user", "content": user_prompt}
                ]
                
                # 准备API调用参数（Ark Chat Completions）
                api_params = {
                ...
                # 调用API（非流式）
                print(f"🤖 [TestModel] 正在调用 Ark Chat Completions: {self.config['model_name']}")
                completion = self.client.chat.completions.create(**api_params)
                reply_content = completion.choices[0].message.content
                print(f"✅ [TestModel] API 调用成功，响应长度: {len(reply_content)} 字符")
...
def run_single_case(script_id: str) -> Dict:
    ...
    try:
        ...
        max_consecutive_failures = 5  # 连续失败5次则终止
        
        if 'epj' in result and 'trajectory' in result['epj']:
            for turn_data in result['epj']['trajectory']:
                test_model_msg = turn_data.get('test_model_message', '')
                if test_model_msg.startswith('（错误：API调用失败'):
                    consecutive_failures += 1
                    if consecutive_failures >= max_consecutive_failures:
                        print(f"\n⚠️ 检测到连续 {consecutive_failures} 次API调用失败")
                        result['status'] = 'api_failure'
                        result['failure_reason'] = f'TestModel API连续失败{consecutive_failures}次'
...
def main():
    ...
    cases_file = BENCHMARK_CASES_FILE
